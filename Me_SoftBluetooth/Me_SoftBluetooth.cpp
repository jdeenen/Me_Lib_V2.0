/******************************************************************************
* File Name          : Me_SoftBluetooth.cpp
* Author             : Steve
* Updated            : Steve
* Version            : V1.0.0
* Date               : 12/12/2012
...
//default port is 4
Me_SoftBluetooth::Me_SoftBluetooth():swSerial(mePort[PORT_4].outsidePin, mePort[PORT_4].innersidePin)
{
	port = PORT_4;
	bluetoothState = BLUETOOTH_PAIRABLE;
}

Me_SoftBluetooth::Me_SoftBluetooth(int portNum):swSerial(mePort[portNum].outsidePin, mePort[portNum].innersidePin)
{
	port = portNum;
	bluetoothState = BLUETOOTH_PAIRABLE;
}

void Me_SoftBluetooth::begin(long baudrate)
{
//	Me_BaseShield::begin();
...
int Me_SoftBluetooth::setName(String name)
{
	int isSuccess = 0;
...
size_t Me_SoftBluetooth::write(uint8_t byte)
{
	return(swSerial.write(byte));
}

int Me_SoftBluetooth::read()
{
	return(swSerial.read());
}

int Me_SoftBluetooth::available()
{
	return(swSerial.available());
}

void Me_SoftBluetooth::flush()
{
	swSerial.flush();
}
...
int Me_SoftBluetooth::checkConnected()
{
